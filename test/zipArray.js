...
describe('#zipArray', () => {
  ...
  */
  it('should create a Maybe', () => {
    const maybe = Maybe.zipArray([Maybe.just('Hello'), Maybe.just('World')]);
  ...
  */
  ...
  it('should signal an error if the zipper throws an error', (done) => {
    const maybe = Maybe.zipArray([Maybe.just('Hello'), Maybe.just('World')], () => undefined);
  ...
  */
  it('should signal success with an array (no zipper function).', (done) => {
    const maybe = Maybe.zipArray([Maybe.just('Hello').delay(100), Maybe.just('World')]);
    maybe.subscribe(
  ...
  */
  it('should signal success with an array with the correct values, consider non-Maybe (no zipper function).', (done) => {
    const maybe = Maybe.zipArray(['Hello', Maybe.just('World')]);
    maybe.subscribe(
      x => (x[0] === 'Hello' && x[1] === 'World' ? done() : done(false)),
      () => done(false),
      () => done(false),
  ...
  */
  it('should signal error if one of the sources is undefined.', (done) => {
    const source = Maybe.zipArray([undefined, Maybe.just('World')]);
    source.subscribe(
      () => done(false),
      () => done(false),
      () => done(),
  ...
  */
  it('should signal error if a source throws error.', (done) => {
    const source = Maybe.zipArray([Maybe.error(new Error('Hello')), Maybe.just('World')]);
    source.subscribe(
      () => done(false),
      () => done(false),
      () => done(),
  ...
  */
  it('should signal complete if a source throws error.', (done) => {
    const source = Maybe.zipArray([Maybe.empty(), Maybe.just('World')]);
    source.subscribe(
      () => done(false),
      () => done(),
      () => done(false),
  ...
  */
  it('should not signal success if cancelled.', (done) => {
    const source = Maybe.zipArray([Maybe.just('Hello').delay(100), Maybe.just('World')]);
